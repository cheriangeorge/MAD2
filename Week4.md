...
* Why?
  ...
  - CPUs are fundamentally imperative - i.e. : you have to give it every step.
 * How?
  - Server tracks state
    - user logged in
    - date/time
  ...
  - Server responds with complete HTML based on persent state
    ...
    - Fully server side
    - Problem: high load on the server
  - Client JS
...
* Vue - directives
  - `v-bind` : one way binding - update variable, reflects on display
    - `v-model`: two way binding - inputs, checkboxes, form data, etc.
  - `v-on` : event binding
    - most common form is click event: `v-on:click`
  - Changes in the model reflects on the display and changes in the actions to reflect in the model.
* Class binding
  - Dynamically modify class of an object: `v-bind:class="classObj"`
...
* Looping
  ...
  - Usage similar to Jinja `{{ }}` templates (called “Mustache” syntax)
  - Examples :
...
* Example 
  - Form with username, password, repeat_password
    - Should repeat_password be in model ?
...
* ViewModel (MVVM pattern)
  - Came from Windows Presentation Foundation (WPF)
  - Yet another "Pattern"
...
* Computed Properties 
  ...
  - Computed "setter" also possible - see [documentation](https://vuejs.org/v2/guide/computed.html#Computed-Setter) for example
* Watcher
...
* Vue Component Structure
  ...
  - Template
    - how to render
    - render functions possible - see [docs](https://vuejs.org/v2/guide/render-function.html)
    - Slots
* Templates
  - `{{ }}` format - similar to Jinja
  - Safety features:
    - will not interpolate text into tags - why?
      - To prevent accidental JS code injection
      - Vue will sanitize the output 
...
* How does Reactivity work?
  ...
  - `Object.defineProperty()`
    ...
    - [Mozilla Docs on Object.defineProperty()](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/defineProperty)
